...
if ("IntersectionObserver" in window && !window.wo_video_observer) {
  window.wo_video_observer = new IntersectionObserver(
    function (entries) {
...
function Wo_SetupAutoLoadPostsObserver() {
  ...
  var more_posts = document.querySelector(
    '#load-more-posts[data-auto-load-only="1"]',
  );
  if (!more_posts) {
    return;
  }
  ...
  window.wo_auto_load_posts_observer = new IntersectionObserver(
    function (entries) {
      entries.forEach(function (entry) {
        if (
          entry.isIntersecting &&
          $("#load-more-posts").length > 0 &&
          scrolled == 0 &&
          !$("body").attr("no-more-posts")
        ) {
          scrolled = 1;
          Wo_GetMorePosts();
        }
  ...
  $.get(
    ...
    function (data) {
      ...
      Wo_progressIconLoader(
        $("#sidebar-user-service-container").find(".refresh"),
      );
    },
  );
}
...
function DeleteUploadedImageById(name, id, post_id) {
  ...
  if ($("#edit-post-form-" + post_id).find(".thumb-image").length < 1) {
    $("#edit-post-form-" + post_id)
      .find("#publisher-post-photos-" + post_id)
      .val("");
  }
}
  ...
  $.get(
    ...
    function (data) {
      post
        .find(".disable-comments")
        .html(
          '<svg viewBox="0 0 24 24"><path fill="currentColor" d="M9,22A1,1 0 0,1 8,21V18H4A2,2 0 0,1 2,16V4C2,2.89 2.9,2 4,2H20A2,2 0 0,1 22,4V16A2,2 0 0,1 20,18H13.9L10.2,21.71C10,21.9 9.75,22 9.5,22V22H9M5,5V7H19V5H5M5,9V11H13V9H5M5,13V15H15V13H5Z" /></svg>&nbsp;&nbsp;&nbsp;<div><b>' +
            data.text +
            "</b><p>" +
            p +
            "</p></div>",
  ...
  $.get(
    ...
    function (data) {
      post
        .find(".pin-post")
        .html(
          '<svg viewBox="0 0 24 24"><path fill="currentColor" d="M16,12V4H17V2H7V4H8V12L6,14V16H11.2V22H12.8V16H18V14L16,12Z" /></svg>&nbsp;&nbsp;&nbsp;<div><b>' +
            data.text +
            "</b><p>" +
            p +
            "</p></div>",
...
function Wo_OpenChatTab(
  ...
  if ($(".chat_" + recipient_id).length > 0 && story_id == 0) {
    ...
    var existing_messages_wrapper = existing_chat.find(
      ".chat-messages-wrapper-" + recipient_id,
    );
    ...
    existing_chat
      .find(".chat-tab-container-" + recipient_id)
      .stop(true, true)
      .show();
    if (existing_messages_wrapper.length > 0) {
      existing_messages_wrapper.scrollTop(
        existing_messages_wrapper[0].scrollHeight,
      );
    }
    return false;
  }
  ...
  $.get(
    ...
    function (data) {
      ...
      if (data.status == 200) {
        ...
        $(".lightbox-container").html(data.html);
        if (
          $('[data-page-type="watch"]').length > 0 &&
          $(".lightbox-container").find("video").length > 0
        ) {
...
function Wo_NextVideo(post_id) {
  ...
  $.get(
    Wo_Ajax_Requests_File(),
    { f: "get_next_video", post_id: post_id, type: type, id: id },
    function (data) {
      if (data.status == 200) {
        document.body.style.overflow = "hidden";
        $(`.plyrr-${data.post_id}`).removeClass(
          `plyrr-${data.post_id} video-container`,
        );
        $(".lightbox-container").html(data.html);
        $(".changer").fadeIn(200);
      }
...
function Wo_PreviousVideo(post_id) {
  ...
  $.get(
    Wo_Ajax_Requests_File(),
    { f: "get_previous_video", post_id: post_id, type: type, id: id },
    function (data) {
      if (data.status == 200) {
        document.body.style.overflow = "hidden";
        $(".lightbox-container").html(data.html);
...
function Wo_PlayAudioCall(type) {
  if (type == "play") {
    document
      .getElementById("calling-sound")
      .play()
      .catch(function (e) {
        console.log(
          "Autoplay blocked: call sound will play after user interaction.",
        );
      });
...
function Wo_PlayVideoCall(type) {
  if (type == "play") {
    document
      .getElementById("video-calling-sound")
      .play()
      .catch(function (e) {
        console.log(
          "Autoplay blocked: video call sound will play after user interaction.",
        );
      });
...
function load_ajax_chat_emojii(id, path) {
  ...
  $.each(emojjii.split("*"), function (key, value) {
    $(".emo-container-" + id).append(
      '<span class="emoji_holder" onclick="Wo_AddEmoToChat(' +
        id +
        ",'" +
        value +
        "');\"><span>" +
...
function load_ajax_chat_group_emojii(id, path) {
  ...
  $.each(emojjii.split("*"), function (key, value) {
    $(".emo-group-container-" + id).append(
      '<span class="emoji_holder" onclick="Wo_AddEmoToGroup(' +
        id +
        ",'" +
        value +
        "');\"><span>" +
...
function load_ajax_chat_page_emojii(id, path) {
  ...
  $.each(emojjii.split("*"), function (key, value) {
    $(".emo-container-" + id).append(
      '<span class="emoji_holder" onclick="Wo_AddEmoToPage(' +
        id +
        ",'" +
        value +
        "');\"><span>" +
...
function load_ajax_message_emojii(path) {
  ...
  $.each(emojjii.split("*"), function (key, value) {
    $(".emo-message-container").append(
      '<span class="emoji_holder" onclick="Wo_AddEmoToMessageInput(\'' +
        value +
        "');\"><span>" +
        value +
        "</span>",
...
function load_ajax_publisher_emojii(path) {
  ...
  $.each(emojjii.split("*"), function (key, value) {
    $(".publisher-box-emooji").append(
      '<span class="emoji_holder" onclick="Wo_AddEmo(\'' +
        value +
        "', '#post-textarea textarea'), Wo_ShowPosInfo();\"><span>" +
        value +
        "</span>",
    );
  });
      ...
      f = function (e) {
        ...
        if (
          ...
          if ((s.contentGhost.append(l), s.container.offset)) {
            ...
            (f > 0 &&
              s.draggableBox.css({ top: e.pageY - o.top + t.y + 5 - f }),
              c > 0 &&
      ...
      (function () {
        (i.settings.className && o.el.addClass(i.settings.className),
          ...
          o.draggable.el.on("clayfy-drop", function (e) {
            (i.el.trigger("clayfy-resizeend"),
              i.settings.callbacks.resizeend());
          }),
...
(function () {
  ...
  var ICON_LIKE =
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 1.97-1.62l1.38-7A2 2 0 0 0 21.66 11H14z"></path><path d="M7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg>';
  var ICON_COMMENT =
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>';
  var ICON_SHARE =
    '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 20 20"><path fill="currentColor" d="M15.815 0C17.575 0 19 1.45 19 3.235s-1.424 3.234-3.185 3.234a3.155 3.155 0 0 1-2.378-1.084l-6.284 3.44c.14.364.216.76.216 1.175c0 .542-.13 1.052-.363 1.501l6.008 3.725a3.177 3.177 0 0 1 2.801-1.695c1.76 0 3.185 1.45 3.185 3.234C19 18.55 17.576 20 15.815 20c-1.76 0-3.184-1.45-3.184-3.235l.003-.138l-6.53-4.046a3.138 3.138 0 0 1-1.92.654C2.425 13.235 1 11.785 1 10s1.424-3.235 3.185-3.235c.852 0 1.626.34 2.197.893l6.382-3.493a3.282 3.282 0 0 1-.133-.93C12.63 1.45 14.055 0 15.815 0Zm0 14.926c-.992 0-1.8.822-1.8 1.84c0 1.017.808 1.839 1.8 1.839c.993 0 1.8-.822 1.8-1.84c0-1.017-.807-1.839-1.8-1.839ZM4.185 8.161c-.993 0-1.8.822-1.8 1.839s.807 1.84 1.8 1.84c.992 0 1.8-.823 1.8-1.84c0-1.017-.808-1.84-1.8-1.84Zm11.63-6.766c-.992 0-1.8.822-1.8 1.84c0 1.017.808 1.839 1.8 1.839c.993 0 1.8-.822 1.8-1.84c0-1.017-.807-1.839-1.8-1.839Z"/></svg>';
  var ICON_SAVE =
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>';
  var ICON_SAVE_FILLED = ICON_SAVE;
  ...
  function createFullscreenButtons(plyrEl) {
    ...
    div
      ...
      .addEventListener("click", function (e) {
        ...
        closeActiveVideoOverlay(false);
        setTimeout(function () {
          var commentBox = $("#post-" + postId).find(".post-comments");
